...
import train_aqm_medfuse_mimic_bp_reg_v10_12_subjectdisjoint_piso_uncertainty_moe_fulltrain_reliabilitybias_protocol as v12
import train_aqm_medfuse_mimic_bp_reg_v10_2_optlong_dualanchor_meta_stack_protocol as meta_script


FINAL_OUTPUT_NAME = "mimic_bp_reg_v10_16_subjectdisjoint_clinicalordinal_biasbalanced_proto"
FINAL_PROTOCOL_ID = "v10.16_subjectdisjoint_clinicalordinal_biasbalanced"
...
_ORIG_V12_SAFETY_CLASS_FUSION_SCORE = v12.safety_class_fusion_score
_ORIG_V12_SEARCH_SAFETY_CLASS_FUSION = v12.search_safety_class_fusion
_ORIG_META_SEARCH_CLASSIFICATION_ARBITER = meta_script.search_classification_arbiter
...
def _cls_metric(metrics: dict, prefix: str, name: str, default: float = 0.0) -> float:
    key_map = {
    ...
    for key in key_map.get(name, (f"cls_{name}_{prefix}", f"{name}_{prefix}", name)):
        if key in metrics:
            return _safe_float(metrics.get(key), default)
    return float(default)


def _cls_class_metric(metrics: dict, prefix: str, metric: str, class_name: str, default: float = 0.0) -> float:
    return _safe_float(metrics.get(f"cls_{metric}_{prefix}_{class_name}"), default)


def _classification_arbiter_total(cfg) -> int:
    return (
        len(tuple(getattr(cfg, "CLS_ARBITER_SCALES", ())))
        * len(tuple(getattr(cfg, "CLS_ARBITER_BETAS", ())))
        * len(tuple(getattr(cfg, "CLS_ARBITER_FLOORS", ())))
        * len(tuple(getattr(cfg, "CLS_ARBITER_AGREE_SHRINKS", ())))
    )
...
def search_classification_arbiter_with_progress(
    arbiter_bundle: dict,
    ...
    rows: List[dict] = []
    best_row = None
    scales = tuple(float(x) for x in cfg.CLS_ARBITER_SCALES)
    betas = tuple(float(x) for x in cfg.CLS_ARBITER_BETAS)
    floors = tuple(float(x) for x in cfg.CLS_ARBITER_FLOORS)
    agree_shrinks = tuple(float(x) for x in cfg.CLS_ARBITER_AGREE_SHRINKS)
    total = len(scales) * len(betas) * len(floors) * len(agree_shrinks)
    progress_every = max(1, int(getattr(cfg, "CLS_ARBITER_PROGRESS_EVERY", 120)))
    started_at = time.perf_counter()
    evaluated = 0
    print(
        f"[v10.16] Final classification arbiter search: 0/{total} candidates "
        f"(progress_every={progress_every})",
        flush=True,
    )
    for arb_scale in scales:
        for arb_beta in betas:
            for arb_floor in floors:
                for arb_agree_shrink in agree_shrinks:
                    probe_row = {
                        "candidate": (
                    ...
                    calib_weight = meta_script.predict_classification_arbiter_weight(
                        arbiter_bundle,
                        calib_selected_prob,
                    ...
                    calib_prob = meta_script.blend_classification_arbiter_prob(
                        calib_selected_prob,
                        calib_stability_prob,
                        calib_weight,
                    )
                    policy_row, _ = meta_script.guided_script.search_policy(calib_prob, y_map["calib"], cfg)
                    clean_weight = meta_script.predict_classification_arbiter_weight(
                        arbiter_bundle,
                    ...
                    noise_weight = meta_script.predict_classification_arbiter_weight(
                        arbiter_bundle,
                    ...
                    ecg_weight = meta_script.predict_classification_arbiter_weight(
                        arbiter_bundle,
                    ...
                    ppg_weight = meta_script.predict_classification_arbiter_weight(
                        arbiter_bundle,
                    ...
                    clean_prob = meta_script.blend_classification_arbiter_prob(
                        query_selected_prob_map["clean"],
                        query_stability_prob_map["clean"],
                        clean_weight,
                    )
                    clean_prob = meta_script._apply_policy_row(clean_prob, policy_row)
                    noise_prob = meta_script.blend_classification_arbiter_prob(
                        query_selected_prob_map["noise"],
                        query_stability_prob_map["noise"],
                        noise_weight,
                    )
                    noise_prob = meta_script._apply_policy_row(noise_prob, policy_row)
                    ecg_prob = meta_script.blend_classification_arbiter_prob(
                        query_selected_prob_map["ecg"],
                        query_stability_prob_map["ecg"],
                        ecg_weight,
                    )
                    ecg_prob = meta_script._apply_policy_row(ecg_prob, policy_row)
                    ppg_prob = meta_script.blend_classification_arbiter_prob(
                        query_selected_prob_map["ppg"],
                        query_stability_prob_map["ppg"],
                        ppg_weight,
                    )
                    ppg_prob = meta_script._apply_policy_row(ppg_prob, policy_row)

                    clean_metrics = meta_script.stage_script.risk_classification_metrics(
                        y_map["clean"],
                    ...
                    noise_metrics = meta_script.stage_script.risk_classification_metrics(
                        y_map["noise"],
                    ...
                    ecg_metrics = meta_script.stage_script.risk_classification_metrics(
                        y_map["ecg"],
                    ...
                    ppg_metrics = meta_script.stage_script.risk_classification_metrics(
                        y_map["ppg"],
                    ...
                    score = meta_script.prev_script.robust_classification_score(
                        clean_metrics,
                        noise_metrics,
                        ecg_metrics,
                        ppg_metrics,
                    ...
                    row = {
                        ...
                        **meta_script.guided_script.class_summary(clean_metrics, "selected_val"),
                    ...
                    if best_row is None or float(row["score"]) > float(best_row["score"]):
                        best_row = row
                    evaluated += 1
                    if evaluated == total or evaluated % progress_every == 0:
                        elapsed = time.perf_counter() - started_at
                        per_candidate = elapsed / max(1, evaluated)
                        eta = per_candidate * max(0, total - evaluated)
                        best_score = float(best_row["score"]) if best_row is not None else float("nan")
                        print(
                            f"[v10.16] Final classification arbiter progress: "
                            f"{evaluated}/{total} ({100.0 * evaluated / max(1, total):.1f}%) | "
                            f"elapsed={_format_eta(elapsed)} | eta={_format_eta(eta)} | "
                            f"best_score={best_score:.6f} | best={best_row['candidate'] if best_row else 'none'}",
                            flush=True,
                        )
...
def tailaware_search_high_bias_calibration_candidates(
    ...
    print(
        f"[v10.16] Exhaustive vectorized high-bias search starting from {processed_local}/{total} "
    ...
    for batch_idx, batch_rows in enumerate(_batched_rows(gen, batch_size), start=1):
        ...
        if batch_idx % flush_every == 0 or processed_local >= total:
            ...
            print(
                f"[v10.16] High-bias search progress: {processed_local}/{total} "
    ...
    print(f"[v10.16] Exhaustive high-bias search finished. Best candidate: {top_rows[0]['candidate']}")
    return top_rows[0], top_rows
...
def search_crisis_tail_debias_candidates(
    ...
    cache_name = "v10_16_crisis_tail"
    signature = _crisis_signature(cfg)
    processed, top_rows, done = _load_cached_rows(cache_name, signature)
    if done and top_rows:
        print(f"[v10.16] Reusing cached exhaustive crisis-tail search results ({len(top_rows)} rows kept).")
        top_rows = sorted(top_rows, key=lambda item: float(item.get("score", 0.0)))
        return top_rows[0], top_rows
    ...
    processed_local = processed
    print(
        f"[v10.16] Exhaustive vectorized crisis-tail search starting from {processed_local}/{total} "
        f"candidates with batch_size={batch_size}."
    )
    for batch_idx, batch_rows in enumerate(_batched_rows(gen, batch_size), start=1):
        ...
        if batch_idx % flush_every == 0 or processed_local >= total:
            elapsed = time.time() - start
            print(
                f"[v10.16] Crisis-tail search progress: {processed_local}/{total} "
                f"({100.0 * processed_local / max(1, total):.1f}%) | elapsed={elapsed / 60.0:.1f} min"
            )
            _save_cached_rows(
                cache_name,
    ...
    print(f"[v10.16] Exhaustive crisis-tail search finished. Best candidate: {top_rows[0]['candidate']}")
    return top_rows[0], top_rows


def guarded_safety_class_fusion_score(metrics: dict, prefix: str, cfg) -> float:
    base = float(_ORIG_V12_SAFETY_CLASS_FUSION_SCORE(metrics, prefix, cfg))
    elevated_name = str(cfg.CLASS_NAMES[1])
    stage1_name = str(cfg.CLASS_NAMES[2])
    stage2_name = str(cfg.CLASS_NAMES[3])
    acc = _cls_metric(metrics, prefix, "acc")
    macro_f1 = _cls_metric(metrics, prefix, "macro_f1")
    balanced_acc = _cls_metric(metrics, prefix, "balanced_acc")
    elevated_f1 = _cls_class_metric(metrics, prefix, "f1", elevated_name)
    stage1_f1 = _cls_class_metric(metrics, prefix, "f1", stage1_name)
    stage2_f1 = _cls_class_metric(metrics, prefix, "f1", stage2_name)
    return float(
        base
        + 2.40 * acc
        + 3.00 * macro_f1
...
def guarded_search_safety_class_fusion(
    query_cls_prob: np.ndarray,
    query_reg_out: dict,
    cfg,
) -> tuple[dict, List[dict]]:
    best, rows = _ORIG_V12_SEARCH_SAFETY_CLASS_FUSION(query_cls_prob, query_reg_out, cfg)
    identity = next((row for row in rows if str(row.get("candidate", "")) == "identity"), None)
    if identity is None:
        return best, rows

    identity_acc = _cls_metric(identity, "selected_val", "acc")
    identity_f1 = _cls_metric(identity, "selected_val", "macro_f1")
    identity_bal = _cls_metric(identity, "selected_val", "balanced_acc")
    best_acc = _cls_metric(best, "selected_val", "acc")
    best_f1 = _cls_metric(best, "selected_val", "macro_f1")
    best_bal = _cls_metric(best, "selected_val", "balanced_acc")

    min_acc_gain = float(getattr(cfg, "SAFETY_CLASS_FUSION_MIN_ACC_GAIN", 0.0))
    min_f1_gain = float(getattr(cfg, "SAFETY_CLASS_FUSION_MIN_F1_GAIN", 0.0))
    max_acc_drop = float(getattr(cfg, "SAFETY_CLASS_FUSION_MAX_ACC_DROP", 0.0))
    max_f1_drop = float(getattr(cfg, "SAFETY_CLASS_FUSION_MAX_F1_DROP", 0.0))

    best_name = str(best.get("candidate", ""))
    accept_best = (
        best_name == "identity"
        or (
            best_acc >= identity_acc - max_acc_drop
            and best_f1 >= identity_f1 - max_f1_drop
            and (
                best_f1 >= identity_f1 + min_f1_gain
                or (best_acc >= identity_acc + min_acc_gain and best_f1 >= identity_f1)
            )
        )
    )
    selected = dict(best if accept_best else identity)
    selected_name = str(selected.get("candidate", ""))
    gate_decision = "accepted" if accept_best else "identity_fallback"

    selected.update(
        {
            "safety_gate_decision": gate_decision,
            "safety_gate_selected": 1,
            "safety_gate_original_best": best_name,
            "safety_gate_original_best_acc": float(best_acc),
            "safety_gate_original_best_macro_f1": float(best_f1),
            "safety_gate_original_best_balanced_acc": float(best_bal),
            "safety_gate_identity_acc": float(identity_acc),
            "safety_gate_identity_macro_f1": float(identity_f1),
            "safety_gate_identity_balanced_acc": float(identity_bal),
            "safety_gate_acc_delta_vs_identity": float(_cls_metric(selected, "selected_val", "acc") - identity_acc),
            "safety_gate_macro_f1_delta_vs_identity": float(_cls_metric(selected, "selected_val", "macro_f1") - identity_f1),
            "safety_gate_balanced_acc_delta_vs_identity": float(_cls_metric(selected, "selected_val", "balanced_acc") - identity_bal),
        }
    )
    ...
    for row in rows:
        annotated = dict(row)
        is_selected = str(annotated.get("candidate", "")) == selected_name
        annotated.update(
            {
                "safety_gate_decision": gate_decision if is_selected else "searched",
                "safety_gate_selected": int(is_selected),
                "safety_gate_original_best": best_name,
                "safety_gate_original_best_acc": float(best_acc),
                "safety_gate_original_best_macro_f1": float(best_f1),
                "safety_gate_original_best_balanced_acc": float(best_bal),
                ...
                "safety_gate_identity_macro_f1": float(identity_f1),
                "safety_gate_identity_balanced_acc": float(identity_bal),
                "safety_gate_acc_delta_vs_identity": float(_cls_metric(annotated, "selected_val", "acc") - identity_acc),
                "safety_gate_macro_f1_delta_vs_identity": float(_cls_metric(annotated, "selected_val", "macro_f1") - identity_f1),
                "safety_gate_balanced_acc_delta_vs_identity": float(_cls_metric(annotated, "selected_val", "balanced_acc") - identity_bal),
            }
        )
        if is_selected:
            annotated.update(selected)
        annotated_rows.append(annotated)

    annotated_rows.sort(
        key=lambda row: (int(row.get("safety_gate_selected", 0)), float(row.get("score", 0.0))),
        reverse=True,
    )
...
def build_nextgen_cfg():
    ...
    cfg.PROTOCOL_NAME = (
        "v10.16 subject-disjoint clinical-ordinal bias-balanced protocol "
        "(reuse v10.13 backbones + exhaustive vector-fast searches + guarded safety fusion)"
    )
    ...
    cfg.BASELINE_ACC_SHORTFALL_WEIGHT = max(float(getattr(cfg, "BASELINE_ACC_SHORTFALL_WEIGHT", 18.0)), 30.0)
    cfg.BASELINE_F1_SHORTFALL_WEIGHT = max(float(getattr(cfg, "BASELINE_F1_SHORTFALL_WEIGHT", 22.0)), 34.0)
    ...
    cfg.BASELINE_ASPIRATIONAL_ACC_WEIGHT = max(float(getattr(cfg, "BASELINE_ASPIRATIONAL_ACC_WEIGHT", 2.40)), 3.4)
    cfg.BASELINE_ASPIRATIONAL_F1_WEIGHT = max(float(getattr(cfg, "BASELINE_ASPIRATIONAL_F1_WEIGHT", 2.80)), 3.8)
    cfg.BASELINE_CRISIS_SBP_TARGET = -4.75
    cfg.BASELINE_CRISIS_DBP_TARGET = -0.75
    cfg.BASELINE_HIGH_SBP_TARGET = -1.75
    cfg.BASELINE_HIGH_DBP_TARGET = 0.0
    cfg.BASELINE_CRISIS_TARGET_WEIGHT = max(float(getattr(cfg, "BASELINE_CRISIS_TARGET_WEIGHT", 3.40)), 4.60)
    cfg.BASELINE_HIGH_TARGET_WEIGHT = max(float(getattr(cfg, "BASELINE_HIGH_TARGET_WEIGHT", 1.45)), 1.25)
    cfg.BASELINE_BIAS_WORSE_TOL_SBP = min(float(getattr(cfg, "BASELINE_BIAS_WORSE_TOL_SBP", 1.0)), 0.65)
    cfg.BASELINE_BIAS_WORSE_TOL_DBP = min(float(getattr(cfg, "BASELINE_BIAS_WORSE_TOL_DBP", 1.0)), 0.45)

    # Bias correction is still allowed to be strong in crisis, but v10.16 avoids the v10.15 over-shift
    # pattern where high-range BP became too positive while classification slipped.
    cfg.CRISIS_SBP_GUARD_ENABLE = True
    cfg.CRISIS_SBP_GUARD_TRIGGER = min(float(getattr(cfg, "CRISIS_SBP_GUARD_TRIGGER", 0.52)), 0.50)
    cfg.CRISIS_SBP_GUARD_QUANTILE = max(float(getattr(cfg, "CRISIS_SBP_GUARD_QUANTILE", 0.98)), 0.982)
    cfg.CRISIS_SBP_GUARD_ABSOLUTE_FLOOR = max(float(getattr(cfg, "CRISIS_SBP_GUARD_ABSOLUTE_FLOOR", 172.0)), 172.0)
    cfg.CRISIS_SBP_GUARD_GAIN = max(float(getattr(cfg, "CRISIS_SBP_GUARD_GAIN", 0.88)), 0.90)
    cfg.CRISIS_SBP_GUARD_MAX_EXTRA_SHIFT = min(max(float(getattr(cfg, "CRISIS_SBP_GUARD_MAX_EXTRA_SHIFT", 18.0)), 16.0), 18.0)
    cfg.CRISIS_DBP_GUARD_ABSOLUTE_FLOOR = max(float(getattr(cfg, "CRISIS_DBP_GUARD_ABSOLUTE_FLOOR", 106.0)), 105.0)
    cfg.CRISIS_DBP_GUARD_GAIN = min(max(float(getattr(cfg, "CRISIS_DBP_GUARD_GAIN", 0.45)), 0.36), 0.45)
    cfg.CRISIS_DBP_GUARD_MAX_EXTRA_SHIFT = min(max(float(getattr(cfg, "CRISIS_DBP_GUARD_MAX_EXTRA_SHIFT", 8.0)), 6.0), 7.0)

    cfg.RISK_GUARD_SHIFT_GRID = (0.45, 0.65, 0.85, 1.05)
    cfg.RISK_GUARD_HIGH_GAIN_GRID = (1.10, 1.35, 1.70)
    cfg.RISK_GUARD_CRISIS_GAIN_GRID = (1.35, 1.80, 2.35, 2.90)
    cfg.RELIABILITY_BIAS_HIGH_GAINS = (0.0, 0.25, 0.45, 0.70)
    cfg.RELIABILITY_BIAS_CRISIS_GAINS = (0.45, 0.70, 0.95, 1.25, 1.55)
    cfg.RELIABILITY_BIAS_CRISIS_THRESHOLDS = (0.08, 0.14, 0.22, 0.30)
    cfg.RELIABILITY_BIAS_CRISIS_FLOOR_SBP = (3.0, 5.0, 7.0, 9.0)
    cfg.CRISIS_TAIL_FUSION_SBP_MARGINS = (-2.0, 0.0, 2.0, 4.0, 6.0, 8.0)
    cfg.CRISIS_TAIL_FUSION_DBP_MARGINS = (-1.5, -0.5, 0.0, 1.0, 2.0)
    cfg.CRISIS_TAIL_FUSION_CRISIS_GAINS = (0.70, 1.00, 1.35, 1.75, 2.20)
    cfg.CRISIS_TAIL_UNDEREST_WEIGHT_SBP = max(float(getattr(cfg, "CRISIS_TAIL_UNDEREST_WEIGHT_SBP", 4.0)), 6.5)
    cfg.CRISIS_TAIL_UNDEREST_WEIGHT_DBP = min(max(float(getattr(cfg, "CRISIS_TAIL_UNDEREST_WEIGHT_DBP", 1.0)), 1.2), 1.6)

    cfg.CLS_ARBITER_SCALES = tuple(dict.fromkeys(tuple(getattr(cfg, "CLS_ARBITER_SCALES", ())) + (0.35, 0.45, 0.55, 0.70, 0.85)))
    cfg.CLS_ARBITER_BETAS = tuple(dict.fromkeys(tuple(getattr(cfg, "CLS_ARBITER_BETAS", ())) + (1.0, 1.4, 1.8, 2.2)))
    cfg.CLS_ARBITER_FLOORS = tuple(dict.fromkeys(tuple(getattr(cfg, "CLS_ARBITER_FLOORS", ())) + (0.0, 0.03, 0.06)))
    cfg.CLS_ARBITER_AGREE_SHRINKS = tuple(dict.fromkeys(tuple(getattr(cfg, "CLS_ARBITER_AGREE_SHRINKS", ())) + (0.05, 0.12, 0.20)))
    cfg.CLS_ARBITER_PROGRESS_EVERY = 120
    cfg.SAFETY_CLASS_FUSION_MAX_WEIGHT = min(float(getattr(cfg, "SAFETY_CLASS_FUSION_MAX_WEIGHT", 0.78)), 0.42)
    cfg.SAFETY_EVIDENTIAL_SCALES = (0.04, 0.08, 0.12, 0.18, 0.26)
    cfg.SAFETY_EVIDENTIAL_STAGE1_BIASES = (0.0, 0.02, 0.04)
    cfg.SAFETY_EVIDENTIAL_STAGE2_BIASES = (0.0, 0.04, 0.08, 0.12)
    cfg.SAFETY_CLASS_FUSION_MIN_ACC_GAIN = 0.0015
    cfg.SAFETY_CLASS_FUSION_MIN_F1_GAIN = 0.0008
    cfg.SAFETY_CLASS_FUSION_MAX_ACC_DROP = 0.0005
...
def generate_v1016_clinicalordinal_audit(output_dir: Path) -> None:
    output_dir = Path(output_dir)
    safety_rows = _read_csv_rows(output_dir / "safety_class_fusion_search.csv")
    selected_safety = next(
        (row for row in safety_rows if _safe_int(row.get("safety_gate_selected"), 0) == 1),
        safety_rows[0] if safety_rows else {},
    )
    bp_rows = _read_csv_rows(output_dir / "tables" / "bp_range_metrics.csv")
    bp_map = {str(row.get("bp_range", "")): row for row in bp_rows}
    crisis_row = bp_map.get("crisis", {})
    high_row = bp_map.get("high", {})
    audit = {
        "protocol_id": FINAL_PROTOCOL_ID,
        "classification_safety_gate": {
            "selected_candidate": str(selected_safety.get("candidate", "")),
            "decision": str(selected_safety.get("safety_gate_decision", "")),
            "original_best": str(selected_safety.get("safety_gate_original_best", selected_safety.get("candidate", ""))),
            "identity_acc": _row_float(selected_safety, "safety_gate_identity_acc"),
            "identity_macro_f1": _row_float(selected_safety, "safety_gate_identity_macro_f1"),
            "original_best_acc": _row_float(selected_safety, "safety_gate_original_best_acc"),
            "original_best_macro_f1": _row_float(selected_safety, "safety_gate_original_best_macro_f1"),
            "selected_acc_delta_vs_identity": _row_float(selected_safety, "safety_gate_acc_delta_vs_identity"),
            "selected_macro_f1_delta_vs_identity": _row_float(selected_safety, "safety_gate_macro_f1_delta_vs_identity"),
        },
        "bp_range_bias": {
            "high": {
                "n": _safe_int(high_row.get("n"), 0),
                "bias_sbp": _row_float(high_row, "bias_sbp"),
                "bias_dbp": _row_float(high_row, "bias_dbp"),
                "mae_mean": _row_float(high_row, "mae_mean"),
            },
            "crisis": {
                "n": _safe_int(crisis_row.get("n"), 0),
                "bias_sbp": _row_float(crisis_row, "bias_sbp"),
                "bias_dbp": _row_float(crisis_row, "bias_dbp"),
                "mae_mean": _row_float(crisis_row, "mae_mean"),
            },
        },
        "method_notes": [
            "Safety classification fusion is still searched exhaustively over the configured grid.",
            "The selected safety-fusion candidate must improve validation accuracy or macro-F1 while staying within the configured drop tolerances; otherwise the identity policy is selected.",
            "High-bias and crisis-tail regression searches remain exact vectorized traversals over their full candidate spaces.",
            "Crisis-range sample count is reported because a very small crisis bin makes test-set bias highly unstable.",
        ],
    }
    _write_json(output_dir / "v10_16_clinicalordinal_biasbalanced_audit.json", audit)
    ...
    runtime_audit = {
        ...
        "search_strategy": {
            "high_bias": {
                ...
                "cache_file": str(_search_cache_path("v10_16_high_bias")),
            },
            "crisis_tail": {
                "candidate_space": _crisis_total(build_nextgen_cfg()),
                "mode": "exact exhaustive search with batched vectorized scoring and resume cache",
                "cache_file": str(_search_cache_path("v10_16_crisis_tail")),
            },
        },
        "backbone_reuse": {
        ...
        "notes": [
            "v10.16 keeps exact high-bias and crisis-tail traversal, but uses clinical-ordinal bias-balanced scoring and guarded safety fusion.",
            ...
            "The output artifact schema stays aligned with the v10.11/v10.13/v10.15 pipeline while adding v10.16-specific audits.",
        ],
    }
    _write_json(Path(output_dir) / "v10_16_vectorfast_runtime_audit.json", runtime_audit)
    generate_v1016_clinicalordinal_audit(output_dir)
...
def main() -> None:
    originals = {
        ...
        "v12_safety_score": v12.safety_class_fusion_score,
        "v12_safety_search": v12.search_safety_class_fusion,
        "meta_safety_score": meta_script.safety_class_fusion_score,
        "meta_safety_search": meta_script.search_safety_class_fusion,
        "meta_cls_arbiter_search": meta_script.search_classification_arbiter,
    }
    try:
        ...
        v12.safety_class_fusion_score = guarded_safety_class_fusion_score
        v12.search_safety_class_fusion = guarded_search_safety_class_fusion
        meta_script.safety_class_fusion_score = guarded_safety_class_fusion_score
        meta_script.search_safety_class_fusion = guarded_search_safety_class_fusion
        meta_script.search_classification_arbiter = search_classification_arbiter_with_progress
        v13.main()
    finally:
        ...
        v12.safety_class_fusion_score = originals["v12_safety_score"]
        v12.search_safety_class_fusion = originals["v12_safety_search"]
        meta_script.safety_class_fusion_score = originals["meta_safety_score"]
        meta_script.search_safety_class_fusion = originals["meta_safety_search"]
        meta_script.search_classification_arbiter = originals["meta_cls_arbiter_search"]
